...
for j in range(10):
    ...
    for t in range(len(D)):
        l_scen1_train_R0, l_scen1_test_R0, l_scen1_train_G, l_scen1_test_G = fun.split_by_index_R0_G(D['t'+str(t)]['R0']['s1'], D['t'+str(t)]['G']['s1'], index_test=index_test_s1[j])  # scen1
        l_scen2_train_R0, l_scen2_test_R0, l_scen2_train_G, l_scen2_test_G = fun.split_by_index_R0_G(D['t'+str(t)]['R0']['s2'], D['t'+str(t)]['G']['s2'], index_test=index_test_s2[j])  # scen2
        m_scen1_train, ld_scen1, il_scen1 = fun.arguments_gp_R0_G(H, phi, l_scen1_train_R0, l_scen1_train_G)
        m_scen2_train, ld_scen2, il_scen2 = fun.arguments_gp_R0_G(H, phi, l_scen2_train_R0, l_scen2_train_G)

        X_train = pd.concat([fun.ltodf(l_scen1_train_R0 + l_scen2_train_R0), fun.ltodf(l_scen1_train_G + l_scen2_train_G)], axis=1)
        X_test = pd.concat([fun.ltodf(l_scen1_test_R0 + l_scen2_test_R0), fun.ltodf(l_scen1_test_G + l_scen2_test_G)], axis=1)
        y_train = np.concatenate((np.repeat(0, len(l_scen1_train_R0)), np.repeat(1, len(l_scen2_train_R0))), axis=None)
        y_test = np.concatenate((np.repeat(0, len(l_scen1_test_R0)), np.repeat(1, len(l_scen2_test_R0))), axis=None)
        ...
        #CNN: keras
        X_train_R0 = X_train.values[...,0:324].reshape(800, 18, 18, 1)
        X_train_G =  X_train.values[...,324:].reshape(800, 18, 18, 1)
        train_images = np.zeros(shape=(800,18,18,2))
        train_images[...,0] = X_train_R0.reshape(800,18,18)
        train_images[...,1] = X_train_R0.reshape(800,18,18)

        X_test_R0 = X_test.values[...,0:324].reshape(200, 18, 18, 1)
        X_test_G =  X_test.values[...,324:].reshape(200, 18, 18, 1)
        test_images = np.zeros(shape=(200,18,18,2))
        test_images[...,0] = X_test_R0.reshape(200,18,18)
        test_images[...,1] = X_test_R0.reshape(200,18,18)

        train_labels =y_train
        test_labels = y_test

        model = models.Sequential()
        model.add(layers.Conv2D(32, (3, 3), activation='relu', input_shape=(18, 18, 2)))
